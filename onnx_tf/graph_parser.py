...
class RNNScopeParser(ScopeParser):
  ...
  def _make_node_info(cls, nodes):
    ...
    for n in nodes:
      ...
      if cls.CELL_NAME in scopes:
        ...
        node_info_holder.cell_dict["type"] = scopes[idx_rnn_cell].replace(
            "_cell", "")
  ...
  def add_kernel_and_bias_concat(cls, nodes, node_dict):
    ...
    for key, value in kb_node_dict.items():
      const_node = TensorflowNode()
      const_node.op_type = "Const"
      const_node.name = "/".join([scope, key, "const_" + get_unique_suffix()])
      const_node.outputs = const_node.get_outputs_names()
      const_node.attr = {
          "value":
          np.array([-1]),
          "dtype":
          data_type.any_dtype_to_onnx_dtype(np.dtype('int32'), None, None),
          "_output_shapes": [1]
      }
      ...
      concat_node.name = "/".join([scope, key, "concat_" + get_unique_suffix()])
      concat_node.inputs = [
          value["gates"].name, value["candidate"].name, const_node.outputs[0]
      ]
      concat_node.outputs = concat_node.get_outputs_names()
      gate_output_shape = node_dict[value["gates"].name].attr["_output_shapes"][
          0]
      candidate_output_shape = node_dict[value["candidate"].name].attr[
          "_output_shapes"][0]
      ...
      concat_node.attr = {"_output_shapes": [concat_output_shapes]}
      nodes.append(concat_node)
    return nodes[-3], nodes[-1]
  ...
  def parse(cls, nodes):
    ...
    for scope in new_cell_nodes:
      kernel, bias = cls.add_kernel_and_bias_concat(new_cell_nodes[scope],
                                                    node_dict)
      node_info_holder.cell_dict["kernel"] = kernel.outputs[0]
      node_info_holder.cell_dict["bias"] = bias.outputs[0]
    ...
    for scope in node_info_holder.nodes:
      ...
      cell_node = cls._make_rnn_node(
          0, node_info_holder.cell_dict, scope, T=dtype)
      cell_node.inputs[0] = input_nodes[0].outputs[0]
  ...
  def _make_major_transpose_nodes(inputs, scope, node_dict, prev_node, post):
    ...
    perm_node.outputs = perm_node.get_outputs_names()
    perm_node.attr = {
    ...
    trans_node.outputs = trans_node.get_outputs_names()
